...
using Veyesys.Core.Domain.Media;
...
namespace Veyesys.Services.Plugins
{
    ...
    public partial class PluginService : IPluginService
    {
        ...
        public PluginService( IHttpContextAccessor httpContextAccessor,
        ...
        public virtual Task<string> GetPluginLogoUrlAsync(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = VePluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{VePluginDefaults.LogoFileName}.{ext}")));
            ...
            var logoUrl = $"{logoPathUrl}{VePluginDefaults.PathName}/" +
                $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{VePluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        throw new VeException(errorMessage);
                    }
                }
            }
