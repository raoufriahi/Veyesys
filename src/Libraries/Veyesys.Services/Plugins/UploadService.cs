...
namespace Veyesys.Services.Plugins
{
    /// <summary>
    ...
    public class UploadService : IUploadService
    {
        ...
        public UploadService(IVeFileProvider fileProvider,
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            ...
            var uploadedItemsFileEntry = archive.Entries
                .FirstOrDefault(entry => entry.Name.Equals(VePluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            if (!string.IsNullOrEmpty(VePluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(VePluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{VePluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{VePluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{VePluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                    ...
                    if (isPluginDescriptor)
                    {
                        ...
                        if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(VeVersion.CURRENT_VERSION))
                            throw new Exception($"This plugin doesn't support the current version - {VeVersion.CURRENT_VERSION}");
                    }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    if (!item.SupportedVersions?.Contains(VeVersion.CURRENT_VERSION) ?? true)
                        continue;
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{VePluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(VePluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{VePluginDefaults.ThemeDescriptionFileName}";
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(VeVersion.CURRENT_VERSION))
                        continue;
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(VePluginDefaults.UploadsTempPath);
        ...
        public virtual void UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(VeCommonDefaults.FaviconAndAppIconsPath, _storeContext.GetActiveStoreScopeConfigurationAsync().Result));
        ...
        public virtual void UploadFavicon(IFormFile favicon)
        {
            ...
            var storeFaviconPath = _fileProvider.GetAbsolutePath(string.Format(VeCommonDefaults.FaviconAndAppIconsPath, _storeContext.GetActiveStoreScopeConfigurationAsync().Result));
        ...
        public virtual void UploadLocalePattern(CultureInfo cultureInfo = null)
        {
            ...
            try
            {
                ...
                var ziplocalePatternPath = getPath(VeCommonDefaults.LocalePatternPath, VeCommonDefaults.LocalePatternArchiveName);
                if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
                    throw new Exception($"Archive '{VeCommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
                ...
                if (!(checkDirectoryExists(VeCommonDefaults.LocalePatternPath, currentCulture.Name) ||
                    checkDirectoryExists(VeCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
                {
                    var cultureToUse = string.Empty;

                    //3. Unpack the archive into a temporary folder
                    ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(VeCommonDefaults.LocalePatternPath, tempFolder));

                    //4. Search in the temp unpacked archive a folder with locales by culture
                    var sourcelocalePath = _fileProvider.Combine(getPath(VeCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
                    if (_fileProvider.DirectoryExists(sourcelocalePath))
                    {
                        cultureToUse = currentCulture.Name;
                    }

                    var sourcelocaleISOPath = _fileProvider.Combine(getPath(VeCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
                    ...
                    {
                        var destlocalePath = getPath(VeCommonDefaults.LocalePatternPath, cultureToUse);
            ...
            finally
            {
                ...
                if (checkDirectoryExists(VeCommonDefaults.LocalePatternPath, tempFolder))
                    _fileProvider.DeleteDirectory(getPath(VeCommonDefaults.LocalePatternPath, tempFolder));
            }
        }
