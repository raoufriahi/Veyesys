...
namespace Veyesys.Services.Common
{
    ...
    /// </summary>
    public partial class VeHttpClient
    {
        #region Fields
        ...
        public VeHttpClient(AdminAreaSettings adminAreaSettings,
            HttpClient client,
        ...
        {
            //configure client
            client.BaseAddress = new Uri("https://www.Veyesys.com/");
            client.Timeout = TimeSpan.FromSeconds(5);
            client.DefaultRequestHeaders.Add(HeaderNames.UserAgent, $"nopCommerce-{VeVersion.CURRENT_VERSION}");
        ...
        public virtual async Task<string> GetCopyrightWarningAsync()
        {
            ...
            var url = string.Format(VeCommonDefaults.NopCopyrightWarningPath,
                store.Url,
        ...
        public virtual async Task<RssFeed> GetNewsRssAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(VeCommonDefaults.NopNewsRssPath,
                VeVersion.CURRENT_VERSION,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        {
            //prepare URL to request
            var url = string.Format(VeCommonDefaults.NopInstallationCompletedPath,
                VeVersion.CURRENT_VERSION,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        public virtual async Task<string> GetExtensionsCategoriesAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(VeCommonDefaults.NopExtensionsCategoriesPath, language).ToLowerInvariant();
        ...
        public virtual async Task<string> GetExtensionsVersionsAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(VeCommonDefaults.NopExtensionsVersionsPath, language).ToLowerInvariant();
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(VeCommonDefaults.NopExtensionsPath,
